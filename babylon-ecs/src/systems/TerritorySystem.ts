...
export class TerritorySystem {
    ...
    private calculateTeamPositions(): void {
        ...
        for (const entity of entities) {
            const teamComponent = entity.getComponent<TeamComponent>(ComponentType.Team);
    ...
    private updateAggressionBonuses(): void {
        ...
        const team1ShouldHaveBonus = team1State.onEnemyTerritory && !team2State.onEnemyTerritory;
        
        // Team2 has bonus if: Team2 is on Team1's side AND Team1 is NOT on Team2's side
        const team2ShouldHaveBonus = team2State.onEnemyTerritory && !team1State.onEnemyTerritory;
     ...
     */
    private updateTeamAggressionBonus(team: TeamTag, shouldHaveBonus: boolean): void {
        const state = this.territoryState.get(team)!;

        if (shouldHaveBonus && !state.hasAggressionBonus) {
            state.hasAggressionBonus = true;
            this.eventBus.emit<AggressionBonusActivatedEvent>(GameEvents.AGGRESSION_BONUS_ACTIVATED, {
                ...createEvent(),
                team,
            ...
            this.eventBus.emit<AggressionBonusDeactivatedEvent>(GameEvents.AGGRESSION_BONUS_DEACTIVATED, {
                ...createEvent(),
                team,
            });
