...
export class VictorySystem {
    ...
    public registerTower(entityId: number, team: TeamTag): void {
        ...
        console.log(`[VictorySystem] Registered tower ${entityId} for team ${team}`);
    }
    ...
    private setupEventListeners(): void {
        ...
        this.unsubscribers.push(
            this.eventBus.on<EntityDestroyedEvent>(GameEvents.ENTITY_DESTROYED, (event) => {
    ...
    private handleBaseDestroyed(destroyedTeam: TeamTag, entityId: number): void {
        const winnerTeam = destroyedTeam === TeamTag.Team1 ? TeamTag.Team2 : TeamTag.Team1;
    ...
    private handleTowerDestroyed(destroyedTeam: TeamTag, entityId: number): void {
        ...
        console.log(`[VictorySystem] Tower ${entityId} (team ${destroyedTeam}) destroyed! Bonus: ${resourceConfig.towerDestructionBonus}`);
    }
     ...
     */
    public forceGameOver(winnerTeam: TeamTag, reason: 'disconnect' | 'forfeit'): void {
        if (this.gameOver) return;
        ...
        console.log(`[VictorySystem] GAME OVER! Team ${winnerTeam} wins by ${reason}!`);
    }
