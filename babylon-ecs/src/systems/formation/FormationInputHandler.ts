...
import type { FormationUnitType, CanAffordCallback, FormationGrid } from "./FormationTypes";
import type {
...
export class FormationInputHandler {
    ...
    private showHoverHighlightForPlacement(
        ...
        const canPlace = this.gridData.canPlaceUnit(playerId, gridX, gridZ, unitType);
        this.hoverPreview.showHoverHighlight(worldPos, worldWidth, worldDepth, canPlace, grid, unitType);
    }
    ...
    private showHoverHighlightForMove(
        ...
        const canMove = isSameCell || this.gridData.canMoveUnit(playerId, fromGridX, fromGridZ, gridX, gridZ, unitType);

        this.hoverPreview.showHoverHighlightForMove(worldPos, worldWidth, worldDepth, canMove);
    }
    ...
    private handleUpdateModeClick(pickResult: PickingInfo | null): void {
        ...
        if (targetCell?.occupied && targetCell.unitType) {
            this.exitUpdateMode(playerId);
            const origin = this.gridData.findUnitOrigin(playerId, gridCoords.x, gridCoords.z);
            if (origin) {
        ...
        if (this.gridData.canMoveUnit(playerId, fromGridX, fromGridZ, gridCoords.x, gridCoords.z, unitType)) {
    ...
    public enterPlacementMode(playerId: string, unitType: FormationUnitType): void {
        ...
        console.log(`[FormationInputHandler] Player ${playerId} entered placement mode for ${unitType}`);
    }
    ...
    public enterUpdateMode(playerId: string, gridX: number, gridZ: number, unitType: FormationUnitType): void {
        ...
        if (grid) {
            ...
            const worldPos = this.gridData.getWorldPosWithOffset(playerId, gridX, gridZ, unitType);
            if (worldPos) {
                this.hoverPreview.highlightSelectedUnit(worldPos, worldWidth, worldDepth);
            }
        }
        ...
        console.log(`[FormationInputHandler] Player ${playerId} entered update mode for ${unitType} at (${gridX}, ${gridZ})`);
    }
