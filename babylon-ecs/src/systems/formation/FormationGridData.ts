...
import type { FormationUnitType, FormationGrid, GridCell, GridCoords, PlacedUnit } from "./FormationTypes";
...
export class FormationGridData {
     ...
     */
    public getUnitGridSize(unitType: FormationUnitType): { width: number; depth: number } {
     ...
     */
    public getWorldPosWithOffset(playerId: string, gridX: number, gridZ: number, unitType: FormationUnitType): Vector3 | null {
        const grid = this.grids.get(playerId);
        if (!grid) return null;
     ...
     */
    private findLanceOrigin(grid: FormationGrid, gridX: number, gridZ: number): GridCoords | null {
     ...
     */
    private findPrismaOrigin(grid: FormationGrid, gridX: number, gridZ: number): GridCoords | null {
        for (let dx = 0; dx >= -1; dx--) {
    ...
    public moveUnit(
        ...
        if (!cell?.occupied || !cell.unitType) return { success: false, unitType: null };
     ...
     */
    public removeUnit(playerId: string, gridX: number, gridZ: number): { success: boolean; originX: number; originZ: number; unitType: FormationUnitType | null } {
        const grid = this.grids.get(playerId);
        if (!grid) return { success: false, originX: 0, originZ: 0, unitType: null };

        const cell = grid.cells[gridX]?.[gridZ];
        if (!cell || !cell.occupied) return { success: false, originX: 0, originZ: 0, unitType: null };

        const unitType = cell.unitType;
        if (!unitType) return { success: false, originX: 0, originZ: 0, unitType: null };

        // Find the origin cell
        const origin = this.findUnitOrigin(playerId, gridX, gridZ);
        if (!origin) return { success: false, originX: 0, originZ: 0, unitType: null };
     ...
     */
    public setCellPreviewMesh(playerId: string, gridX: number, gridZ: number, mesh: import("@babylonjs/core").Mesh): void {
        const grid = this.grids.get(playerId);
